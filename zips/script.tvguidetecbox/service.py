...
try:
    ADDON = xbmcaddon.Addon('script.tvguidetecbox')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.tvguidetecbox)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.tvguidetecbox] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)